...
import {
  ...
  ShoppingCart, DollarSign, BarChart3, Settings, ChevronLeft, ChevronRight, Menu, X
} from 'lucide-react';
...
export default function NexusLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const handleResize = () => { const mobile = window.innerWidth <= 768; setIsMobile(mobile); if (!mobile) setMobileOpen(false); }; window.addEventListener('resize', handleResize); return () => window.removeEventListener('resize', handleResize); }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", position: "relative" }}>
      ...
      <style>{`
        .nx-sb { width: ${sidebarOpen ? '240px' : '64px'}; transition: width 0.3s cubic-bezier(0.4,0,0.2,1); background: rgba(255,255,255,0.92); backdrop-filter: blur(20px); border-right: 1px solid var(--border); display: flex; flex-direction: column; position: fixed; top: 0; left: 0; bottom: 0; z-index: 30; overflow: hidden; }
        ...
        @media (max-width: 768px) {
          .nx-sb { width: 280px !important; transform: translateX(${mobileOpen ? '0' : '-100%'}); transition: transform 0.3s cubic-bezier(0.4,0,0.2,1); z-index: 50 !important; }
          .nx-main { margin-left: 0 !important; padding: 1rem !important; padding-top: 3.5rem !important; }
          .nx-mb { display: flex !important; }
          .nx-overlay { display: ${mobileOpen ? 'block' : 'none'} !important; }
        }
      `}</style>

      {/* Mobile overlay backdrop */}
      <div className="nx-overlay" onClick={() => setMobileOpen(false)} style={{ display: 'none', position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 45 }}></div>
      ...
      <div className="nx-mb" style={{ display: 'none', position: 'fixed', top: 0, left: 0, right: 0, height: '50px', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border)', zIndex: 35, alignItems: 'center', padding: '0 1rem', gap: '0.5rem' }}>
        <button onClick={() => setMobileOpen(true)} className="btn-icon" style={{ background: 'var(--bg-accent)' }}><Menu size={20} /></button>
        ...
        {mobileOpen && <button onClick={() => setMobileOpen(false)} className="btn-icon" style={{ background: 'transparent' }}><X size={20} /></button>}
      </div>
      ...
      <aside className="nx-sb" style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '0.85rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', borderBottom: '1px solid var(--border)', minHeight: '56px' }}>
          <button onClick={() => { isMobile ? setMobileOpen(false) : setSidebarOpen(!sidebarOpen); }} className="btn-icon" style={{ flexShrink: 0, background: 'var(--bg-accent)', width: '30px', height: '30px' }} title={sidebarOpen ? 'Colapsar' : 'Expandir'}>
            {isMobile ? <X size={16} /> : (sidebarOpen ? <ChevronLeft size={16} /> : <ChevronRight size={16} />)}
          </button>
          {sidebarOpen && (
            <>
              <Link href="/" style={{ display: 'flex', alignItems: 'center', lineHeight: 0 }} onClick={() => setMobileOpen(false)}>
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`nx-item ${isActive ? 'active' : ''}`}
                ...
                style={{ padding: '0.6rem 0.75rem', margin: '0.1rem 0.5rem' }}>
                <Icon size={19} style={{ flexShrink: 0 }} />
                <span className="nx-label">{item.label}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div style={{ padding: '0.6rem 0.5rem', borderTop: '1px solid var(--border)' }}>
          <button onClick={() => { handleLogout(); setMobileOpen(false); }}
            style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', width: '100%', padding: '0.6rem 0.75rem', borderRadius: '10px', border: 'none', background: 'var(--danger-soft)', color: 'var(--danger)', cursor: 'pointer', fontWeight: 650, fontSize: '0.8rem', fontFamily: 'var(--font)', transition: 'all 0.2s' }}
